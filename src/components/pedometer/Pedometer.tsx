...
const Pedometer:FC<{message:string,onCross:()=>void}> = ({message,onCross}) => {
  ...
  const startStepCounter = () => {
    startStepCounterUpdate(new Date(),(data)=>{
      const parseData = parseStepData(data);
      addSteps(parseData.steps,parseData.distance)
    })
  }
  ...
  useEffect(()=>{
    if(stepCount >= dailyGoal){
      playTts("You have completed you daily goal.")
    }else{
      startStepCounter()
    }
    // unmounting
  ...
  return (
    <View style={styles.container}>
      <TouchableOpacity style={styles.cross} onPress={()=>{
        Alert.alert("Step counter stopped.")
        stopStepCounter()
        onCross()
        }}>
